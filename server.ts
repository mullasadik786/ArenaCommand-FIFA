...
app.post('/api/stadium/incident', async (req, res) => {
  ...
  // FIFA-Compliant Incident Classification
  let fifaTag = "FIFA-SEC-CLASS-101-GEN";
  let fifaDesc = "Standard Venue Operations & Crowd Flow";
  const combText = `${type} ${description} ${photoAnalysis || ''}`.toLowerCase();

  if (combText.includes('fight') || combText.includes('scuffle') || combText.includes('riot') || combText.includes('altercation') || combText.includes('violence') || combText.includes('stewards') || combText.includes('security')) {
    fifaTag = "FIFA-SEC-CLASS-105-VIO";
    fifaDesc = "Access Control & Spectator Conduct Violation";
  } else if (combText.includes('slip') || combText.includes('fall') || combText.includes('spill') || combText.includes('leak') || combText.includes('hazard')) {
    fifaTag = "FIFA-SEC-CLASS-104-HAZ";
    fifaDesc = "Spectator Safety & Slip/Fall Hazard";
  } else if (combText.includes('crack') || combText.includes('structural') || combText.includes('damage') || combText.includes('collapse')) {
    fifaTag = "FIFA-SEC-CLASS-103-STR";
    fifaDesc = "Facilities Integrity & Structural Defect";
  } else if (combText.includes('medical') || combText.includes('distress') || combText.includes('heart') || combText.includes('unconscious') || combText.includes('injury') || combText.includes('cardiac')) {
    fifaTag = "FIFA-SEC-CLASS-102-MED";
    fifaDesc = "Life-Safety Emergency & First Aid Response";
  } else if (combText.includes('bag') || combText.includes('policy') || combText.includes('clear bag') || combText.includes('size') || combText.includes('compliance')) {
  ...
  // Visual array parsing and prioritization rules
  if (combText.includes('crack') || combText.includes('structural') || combText.includes('damage') || combText.includes('collapse')) {
    severity = 'critical';
    recommendations = `🚨 CRITICAL STRUCTURAL RISK IDENTIFIED VIA LoRA PHOTOMETRIC LAYERS [${fifaTag}]:
- IMMEDIATELY initiate localized evacuation of Sector ${sectorId.toUpperCase()}.
...
- Barricade the immediate danger radius. Compliance: FIFA-SEC-ANNEX-2026.\n\n` + recommendations;
  } else if (combText.includes('medical') || combText.includes('distress') || combText.includes('heart') || combText.includes('unconscious') || combText.includes('injury')) {
    severity = 'critical';
    recommendations = `🚑 MEDICAL EMERGENCY PRIORITIZED (HIGH RISK PATIENT) [${fifaTag}]:
- Dispatch FIFA Red Cross triage squad immediately to Sector ${sectorId.toUpperCase()} seating coordinates.
- Bring portable AED.
- Coordinate Gate A (North Access) ambulance transfer pathway. Compliance: FIFA-SEC-ANNEX-2026.\n\n` + recommendations;
  } else if (combText.includes('debris') || combText.includes('blocker') || combText.includes('trash')) {
    // Explicitly lower risk to allow structural/medical focus
    severity = 'low';
    recommendations = `🧹 ROUTINE DEBRIS BLOCKER [${fifaTag}]:
- Assign cleaning crew to sweep block. No emergency dispatch required. Compliance: FIFA-SEC-ANNEX-2026.\n\n` + recommendations;
  } else {
    recommendations = `Compliance Reference: [${fifaTag} - ${fifaDesc}] under protocol FIFA-SEC-ANNEX-2026.\n\n` + recommendations;
  }
